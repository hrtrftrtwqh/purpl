...
unsigned int nStakeMaxAge = 60 * 60 * 20;
unsigned int nModifierInterval = 5 * 30;

// fimxe
int nCoinbaseMaturity = 26;
...
const string strMessageMagic = "Whistle Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+6) - GetDepthInMainChain());
}
...
static const int64_t nTargetTimespan = 20 * 60 * 60;  // 20 hours
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Whistle", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Whistleblowing platfrom for selling rumors is open";
        CTransaction txNew;
        txNew.nTime = 1448113379;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("6e37fc4b6cee1254d1f0bf788fb27fbc533f22f914c8f950117652e80b48b986"));
        block.print();
